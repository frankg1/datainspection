...
#orcConfig ='crnop/pc#yvc@10.164.25.65:52522/lrnop'
orcConfig ='crnop/pc#yvc@134.192.245.134:1521/lrnop'
...
class Inspection():
    ...
    def getVirtualSiteOfHZ(self,AddressList,PARTITION=PARTITION_IN_CLASS):
        ...
        for address in AddressList:
            ...
            instr=instr[0:len(instr)-1]
            instr+=")"
            sql1="SELECT DISTINCT ADDRESS,NECUR.LONGITUDE as site_long,NECUR.LATITUDE AS site_lat,NECUR.OID as OIDOFCELL,CELLID,COVER_TYPE,NECUR.MULT_FRE_ANT_TYPE AS ANT_TYPE,START_TIME,GRID.VENDOR AS VENDOR,NECUR.CITY_NAME AS CITY_NAME,NECUR.LC_NAME,NECUR.MULT_FRE_ANT_TYPE AS ANT_TYPE,NECUR.ENODEBID,NECUR.CELLID,NECUR.PCI,NECUR.ANT_AZIMUTH FROM NECUR_CELL_L_MR_tp NECUR LEFT JOIN LTE_MRO_GRID_CELL_d PARTITION(P"+PARTITION+") GRID ON NECUR.OID = GRID.OID WHERE GRID.RSRP_AVG > -100 AND NECUR.OID IN "+instr
            cursor.execute(sql1)
            querylist = cursor.fetchmany(100)
    ...
    def getrsrpmaxfromonecell(self,OID,PARTITION=PARTITION_IN_CLASS):
        ...
        #sql="SELECT ADDRESS,NECUR.LONGITUDE as site_long,NECUR.LATITUDE AS site_lat,NECUR.OID as OIDOFCELL,CELLID,COVER_TYPE,GRID_ID,substr(GRID.GRID_ID, 0, instr(GRID.GRID_ID, '|', 1, 1) - 1) / 100000 long_lt,substr(GRID.GRID_ID,instr(GRID.GRID_ID, '|', 1, 1) + 1,length(GRID.GRID_ID)) / 100000 lat_lt,GRID.RSRP_AVG AS RSRP_AVG,NECUR.MULT_FRE_ANT_TYPE AS ANT_TYPE,START_TIME,GRID.VENDOR AS VENDOR,NECUR.CITY_NAME AS CITY_NAME,NECUR.LC_NAME,NECUR.MULT_FRE_ANT_TYPE AS ANT_TYPE,NECUR.ENODEBID,NECUR.CELLID,NECUR.PCI,NECUR.ANT_AZIMUTH,NECUR.CITY_ID FROM NECUR_CELL_L_MR_tp NECUR LEFT JOIN LTE_MRO_GRID_CELL_d PARTITION(P"+PARTITION+") GRID ON NECUR.OID = GRID.OID WHERE GRID.RSRP_AVG > -100 AND NECUR.OID ='"+OID+"'"
        sql="SELECT GRID_ID,substr(GRID.GRID_ID, 0, instr(GRID.GRID_ID, '|', 1, 1) - 1) / 100000 long_lt,substr(GRID.GRID_ID,instr(GRID.GRID_ID, '|', 1, 1) + 1,length(GRID.GRID_ID)) / 100000 lat_lt,GRID.RSRP_AVG AS RSRP_AVG FROM  LTE_MRO_GRID_CELL_d PARTITION(P"+PARTITION+") GRID  WHERE RSRP_AVG > -100 AND OID ='"+OID+"'"
    ...
    def getnummaxfromonecell(self,OID,PARTITION=PARTITION_IN_CLASS):
        ...
        #sql="SELECT ADDRESS,site_long,site_lat,OIDOFCELL,CELLID,COVER_TYPE,GRID_ID,long_lt,lat_lt,RSRP_AVG FROM  PARTITION(P20190902) WHERE OIDOFCELL= "+"'"+OID+"'"
        sql="SELECT GRID_ID,substr(GRID.GRID_ID, 0, instr(GRID.GRID_ID, '|', 1, 1) - 1) / 100000 long_lt,substr(GRID.GRID_ID,instr(GRID.GRID_ID, '|', 1, 1) + 1,length(GRID.GRID_ID)) / 100000 lat_lt,GRID.RSRP_AVG AS RSRP_AVG FROM  LTE_MRO_GRID_CELL_d PARTITION(P"+PARTITION+") GRID  WHERE RSRP_AVG > -100 AND OID ='"+OID+"'"
        #sql="SELECT GRID_ID,substr(GRID.GRID_ID, 0, instr(GRID.GRID_ID, '|', 1, 1) - 1) / 100000 long_lt,substr(GRID.GRID_ID,instr(GRID.GRID_ID, '|', 1, 1) + 1,length(GRID.GRID_ID)) / 100000 lat_lt,GRID.RSRP_AVG AS RSRP_AVG FROM  LTE_MRO_GRID_CELL_d PARTITION(P"+PARTITION+") GRID  WHERE RSRP_AVG > -100 AND OID ='"+OID+"'"+"AND RSRP_AVG=(SELECT MAX(TO_NUMBER(RSRP_AVG)) FROM LTE_MRO_GRID_CELL_d PARTITION (P"+PARTITION+") WHERE RSRP_AVG > -100 AND OID = '"+OID+"')"
        #print(OID,'查询本小区栅格  rsrp最大：',sql)
